...
def encode_data(data,data_dim):
    ...
    idx1 = tf.range(data.shape[0]) 
    idx2 = tf.range(data.shape[1]) 

    ceil_index = tf.cast(ceil_index,tf.int32)
    floor_index = tf.cast(floor_index,tf.int32)
    
    idx1 = tf.reshape(tf.tile(tf.reshape(idx1,[-1,1]),[1,data.shape[1]]),[-1])
    idx2 = tf.tile(idx2,[data.shape[0]])
    idx_ceil = tf.stack((idx1, idx2,ceil_index),axis=-1)
    idx_floor = tf.stack((idx1, idx2,floor_index),axis=-1)
    
    softmax_distr_ceil =  tf.scatter_nd(idx_ceil,data_ceil_prob,(tf.shape(data)[0], tf.shape(data)[1], quantization_channels))
    softmax_distr_floor =  tf.scatter_nd(idx_floor,data_floor_prob,(tf.shape(data)[0],tf.shape(data)[1], quantization_channels))
